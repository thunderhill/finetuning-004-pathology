...
\author{
P.~Mahipal Rao\\
\texttt{p.mahipal@tcs.com}
\and
Sai Kalyan Guddeti\\
\texttt{saikalyan.guddeti@tcs.com}\\[0.3em]
...
GPU (192\,GB HBM3), enabling full fine-tuning of a 303M-parameter ViT-L/16 at large batch size
...
stain augmentation recovers 30.5 points of out-of-distribution accuracy for ViT-L/16
($53.9\%\rightarrow84.4\%$)---equivalently, it shrinks the clean-to-OOD accuracy drop from
44.0 to 12.9 points, a $3.4\times$ smaller robustness gap---at negligible cost to
in-distribution accuracy (97.9\% vs.\ 97.3\%). The learned invariance further transfers to a
mechanistically disjoint corruption family (JPEG compression, blur, downscaling, colour
casts), where accuracy falls only 4.6 points. All five model configurations train end-to-end
on a single MI300X, and we report measured throughput and peak-memory figures.
...
stain augmentation on deployment robustness, recovering 30.5 points of OOD accuracy
($53.9\%\rightarrow84.4\%$; a $3.4\times$ smaller clean-to-OOD gap) at a cost of only 0.6
points of in-distribution accuracy, and verifying that the gain transfers to a disjoint
corruption family. (3) A systems account of training large ViTs in
...
robustness (Section~\ref{sec:ood}). Even after this normalization the benchmark retains a strong
colour bias: a logistic-regression classifier using only per-image colour statistics (the mean and
standard deviation of each RGB channel, six features in total) attains 72\% accuracy on the
nine-class validation set---far above the 11\% chance rate---confirming that colour alone is a
powerful shortcut on this dataset and motivating explicitly colour-invariant training.
...
of NaNs. The capacity advantage allows full fine-tuning of the 303M-parameter ViT-L/16 at batch
...
CTransPath \citep{wang2022ctranspath}         & Swin (SSL)               & 96.5 \\
\midrule
...
\begin{figure}[h]
\centering
\includegraphics[width=0.82\linewidth]{figures/ablation_bar.png}
\caption{Clean vs.\ OOD accuracy for ViT-L/16 trained with and without HED stain augmentation.
Stain augmentation reduces the OOD accuracy drop from 44.0 to 12.9 points---a $3.4\times$
reduction---at the cost of only 0.6 points of in-distribution accuracy.}
\label{fig:ablation}
\end{figure}
...
($\theta{=}0.05$). Accuracy (\%) on clean and stain-shifted CRC-VAL-HE-7K. Bold indicates
the most OOD-robust model.}
\label{tab:ood_arch}
\begin{tabular}{lccc}
\toprule
...
\paragraph{Robustness to a disjoint corruption family.}
A natural objection is that the OOD set above perturbs the same HED axis used for augmentation, so
the gain might reflect inversion of the training transform rather than genuine invariance. To test
this, we evaluate the stain-augmented ViT-L/16 on a second OOD set generated by a mechanistically
disjoint corruption family---JPEG compression, Gaussian blur, resolution downscaling, RGB-space
colour casts, and gamma shifts---none of which appears during training and none of which operates in
HED space. Accuracy falls only 4.6 points ($97.3\%\rightarrow92.7\%$), well inside the
12.9-point HED-shift drop, indicating that the learned invariance generalizes beyond the augmented
axis to unseen acquisition artefacts rather than merely undoing the training perturbation.

\subsection{Explainability}
Figure~\ref{fig:gradcam} shows Grad-CAM overlays for one representative patch per class. For tumour
(TUM) the activation concentrates on the crowded epithelial nuclei that define adenocarcinoma, and
for stroma (STR) it follows the elongated connective-tissue bands rather than the surrounding
background---consistent with the structures a pathologist uses to separate the two, and with the
confusion matrix, where the largest residual error is the biologically adjacent STR$\to$MUS
confusion (Figure~\ref{fig:cm}). Adipose and background attributions are diffuse, as expected for
near-textureless classes, while lymphocyte maps highlight dense small-nucleus clusters. The overlays
thus indicate that high-confidence predictions rest on morphological evidence rather than staining
colour or edge artefacts, supporting the intended effect of the augmentation.
...
compilation~\citep{ansel2024pytorch2} reduces Python dispatch overhead. Table~\ref{tab:systems}
reports measured training-step performance for the 303M-parameter ViT-L/16 at batch size 512 in
native \texttt{bfloat16}. The 192\,GB HBM3 capacity is the enabling factor: a full training step at
batch 512 reaches a peak memory reservation of 90.4\,GB, which already exceeds the 80\,GB capacity
of a contemporary high-end accelerator---fitting the same configuration elsewhere would force
gradient checkpointing or a smaller batch. Enabling \texttt{torch.compile} (reduce-overhead) raises
throughput from 640 to 725 images/s (a $1.13\times$ speedup) at no additional memory cost.

\begin{table}[h]
\centering
\caption{Measured MI300X training-step performance: ViT-L/16 (303M parameters), batch 512, native
\texttt{bfloat16}, $224\times224$ inputs. Peak HBM is the maximum reserved memory over the step.}
\label{tab:systems}
\begin{tabular}{lccc}
\toprule
Execution mode & Throughput (img/s) & Step time (ms) & Peak HBM (GB) \\
\midrule
Eager                       & 640 & 800 & 90.4 \\
\texttt{torch.compile}      & 725 & 706 & 89.6 \\
\bottomrule
\end{tabular}
\end{table}
...
Our robustness claim rests on \emph{synthetic} corruptions; while HED perturbation is an accepted
proxy for inter-laboratory variation and our second, disjoint corruption family probes a different
axis, neither can fully reproduce scanner- and protocol-specific artefacts, and genuine multi-centre
validation remains necessary before clinical use. The study
...
cross-appearance generalization, recovering 30.5 points of ViT-L/16's out-of-distribution accuracy
($53.9\%\rightarrow84.4\%$; equivalently a $3.4\times$ smaller clean-to-OOD gap) at a cost of only
0.6 points of in-distribution accuracy, with the gain transferring to a disjoint corruption family,
while gradient-based explanations confirm morphologically grounded predictions. Future work includes
multi-centre validation with real inter-laboratory stain diversity and extension to slide-level
diagnosis via multiple-instance learning over whole-slide images.
